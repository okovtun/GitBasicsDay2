...
const int ROWS = 8;	//ROW - ñòðîêà
const int COLS = 7; //COLUMN - ñòîëáåö
...
void Sort(int arr[], const int n)
{
	//Ñîðòèðîâêà îäíîìåðíîãî ìàññèâà:
	for (int i = 0; i < n; i++)	//Âûáèðàåò ýëåìåíò, â êîòîðûé íóæíî ïîìåñòèòü ìèíèìàëüíîå çíà÷åíèå
	{
		for (int j = i + 1; j < n; j++)	//Ïåðåáèðàåò ýëåìåíòû, â ïîèñêàõ ìèíèìàëüíîãî çíà÷åíèÿ
		{
			if (arr[j] < arr[i])
			{
				arr[i] ^= arr[j];
